...
if ( ! isset( $arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample'] ) ) {	// for the fist time of calling the template.
	$arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_avatar_size'] = 48;
	$arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_width'] = 100;
	$arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_width_unit'] = '%';	
	$arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_height'] = 400;
	$arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_height_unit'] = 'px';
	update_option( FetchTweets_Commons::AdminOptionKey, $arrOptions );
}

$arrArgs['avatar_size'] = isset( $arrArgs['avatar_size'] ) ? $arrArgs['avatar_size'] : $arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_avatar_size'];
$arrArgs['width']		= isset( $arrArgs['width'] ) ? $arrArgs['width'] : $arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_width'];
$arrArgs['width_unit']	= isset( $arrArgs['width_unit'] ) ? $arrArgs['width_unit'] : $arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_width_unit'];
$arrArgs['height']		= isset( $arrArgs['height'] ) ? $arrArgs['height']: $arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_height'];
$arrArgs['height_unit']	= isset( $arrArgs['height_unit'] ) ? $arrArgs['height_unit'] : $arrOptions['fetch_tweets_templates']['fetch_tweets_template_sample']['fetch_tweets_template_sample_height_unit'];
$strWidth = $arrArgs['width'] . $arrArgs['width_unit'];
$strHeight = $arrArgs['height'] . $arrArgs['height_unit'];
?>

<div class='fetch-tweets-sample-container' style='max-width:<?php echo $strWidth; ?>; max-height:<?php echo $strHeight; ?>;'>
	
	<div class='fetch-tweets-sample-heading'>
		<?php if ( $arrArgs['avatar_size'] > 0 ) : ?>
		<div class='fetch-tweets-sample-profile-image' style="width:<?php echo $arrArgs['avatar_size'];?>px;">
	...
	<?php 
		...
		$strRetweetClassProperty = isset( $arrDetail['retweeted_status']['text'] ) ? 'fetch-tweets-sample-retweet' : '';
		
	?>
    <div class='fetch-tweets-sample-item <?php echo $strRetweetClassProperty; ?>' >
